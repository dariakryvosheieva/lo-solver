...
from rule import EmptySpecification, ConstantPhoneme, MetathesisFocus
from unit_tests.test_generate_focus_candidates import normalize_candidates
...
def test_insertion_1():
    ...
    candidates = RuleSolver(
        ...
        order=order,
        morphemes=morphemes,
    )._generate_change_candidates(
        float("inf"), "insertion", EmptySpecification(), freq_cap=10
    )
    assert normalize_candidates(candidates) == ["n", "ŋ"]
...
def test_insertion_2():
    ...
    assert normalize_candidates(candidates) == ["n", "1", "-1", "ɕ", "r"]
...
def test_deletion_1():
    ...
    candidates = RuleSolver(
        ...
        float("inf"), "deletion", ConstantPhoneme("a"), freq_cap=10
    )
...
def test_deletion_2():
    ...
    candidates_a = solver._generate_change_candidates(
        float("inf"), "deletion", ConstantPhoneme("a"), freq_cap=10
    )
    candidates_u = solver._generate_change_candidates(
        float("inf"), "deletion", ConstantPhoneme("u"), freq_cap=10
    )
    candidates_i = solver._generate_change_candidates(
        float("inf"), "deletion", ConstantPhoneme("i"), freq_cap=10
    )
    assert normalize_candidates(candidates_a) == ["Ø"]
    assert normalize_candidates(candidates_u) == ["Ø"]
    assert normalize_candidates(candidates_i) == ["Ø"]
...
def test_substitution_1():
    ...
    candidates_ts = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("t^s"), freq_cap=10
    )
    candidates_t = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("t"), freq_cap=10
    )
    assert normalize_candidates(candidates_ts) == [
        "d^z",
        "[ +alveolar ]",
        "[ -nasal ]",
        "[ +voice ]",
...
def test_substitution_2():
    ...
    candidates_p = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("p"), freq_cap=10
    )
    candidates_t = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("t"), freq_cap=10
    )
    candidates_k = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("k"), freq_cap=10
    )
...
def test_integration():
    ...
    candidates_insertion = solver._generate_change_candidates(
        float("inf"), "insertion", EmptySpecification(), freq_cap=10
    )
    candidates_deletion = solver._generate_change_candidates(
        float("inf"), "deletion", ConstantPhoneme("i"), freq_cap=10
    )
    candidates_substitution = solver._generate_change_candidates(
        float("inf"), "substitution", ConstantPhoneme("u"), freq_cap=10
    )
    restricted_candidates = solver._generate_change_candidates(
        ConstantPhoneme("u").cost() + 1.0,
        "substitution",
        ConstantPhoneme("u"),
        freq_cap=10,
    )
    candidates_wrong_focus = solver._generate_change_candidates(
        float("inf"), "insertion", ConstantPhoneme("u"), freq_cap=10
    )
    assert normalize_candidates(candidates_insertion) == []
    assert normalize_candidates(candidates_deletion) == ["Ø"]
...
def test_metathesis():
    ...
    assert normalize_candidates(candidates) == ["MET"]
